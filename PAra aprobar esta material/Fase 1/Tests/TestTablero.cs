...
namespace Tests
{
    ...
    public class TestTablero
    {
        ...
        public void TestCtor()
        {
            ...
            TipoCasilla esquina2 = tab.GetCasilla(0, 5);
            TipoCasilla esquina3 = tab.GetCasilla(6, 0);
            TipoCasilla esquina4 = tab.GetCasilla(6, 5);

            // Assert
            Assert.That(esquina1, Is.EqualTo(TipoCasilla.VACIA), "ERROR: nada más construirse un tablero debe tener las esquinas vacías");
            Assert.That(esquina2, Is.EqualTo(TipoCasilla.VACIA), "ERROR: nada más construirse un tablero debe tener las esquinas vacías");
            Assert.That(esquina3, Is.EqualTo(TipoCasilla.VACIA), "ERROR: nada más construirse un tablero debe tener las esquinas vacías");
            Assert.That(esquina4, Is.EqualTo(TipoCasilla.VACIA), "ERROR: nada más construirse un tablero debe tener las esquinas vacías");
        ...
        public void TestColocaFichaDoble()
        {
            ...
            // Assert
            Assert.That(color, Is.EqualTo(TipoCasilla.ROJA), "ERROR: la segunda llamada a ColocaFicha con la misma posición no debe tener efecto.");
        ...
        // CaeFicha devuelve true cuando puede y false cuando la columna
        // está completa
        [Test]
        public void TestCaeFichaColumnaCompleta()
        {
            // Arrange
            Tablero tab = new Tablero();
            for (int i = 0; i < 5; ++i)
            ...
            // Act
            bool ultimaQueEntra = tab.CaeFicha(0, TipoCasilla.AMARILLA);
            bool noEntra = tab.CaeFicha(0, TipoCasilla.AMARILLA);

            // Assert
            Assert.That(ultimaQueEntra, Is.True, "ERROR: CaeFicha debe devolver true si la ficha entra.");
            Assert.That(noEntra, Is.False, "ERROR: CaeFicha debe devolver false cuando la ficha no entra.");
        }
